...
def generate_estimation_report(estimated_blocks, composites_data, idw_params, search_params, block_sizes, 
    ...
    if estimated_blocks:
        ...
        if density_method == "constant":
            ...
            block_volumes = [block['size_x'] * block['size_y'] * block['size_z'] for block in estimated_blocks]
            avg_density = sum(block.get('density', density_value) * vol for block, vol in zip(estimated_blocks, block_volumes)) / sum(block_volumes)
...
def calculate_tonnage_grade(blocks, density_method="constant", density_value=2.7, method="above", cutoff_value=None, cutoff_min=None, cutoff_max=None):
    ...
    for cutoff in cutoffs:
        ...
        for block in filtered_blocks:
            ...
            total_metal += block_tonnage * block['value']
...
def plot_3d_model_with_cubes(blocks, composites, envelope_data=None, block_scale=0.9):
    ...
    if blocks:
        ...
        if len(blocks) > 2000:
            st.warning(f"Le modèle contient {len(blocks)} blocs. Pour une meilleure performance, seuls 2000 blocs sont affichés.")
            x_vals = x_vals[:2000*8]  # 8 sommets par bloc
            y_vals = y_vals[:2000*8]
            z_vals = z_vals[:2000*8]
            intensity = intensity[:2000*8]
            i_vals = i_vals[:2000*12]  # 12 triangles par bloc
            j_vals = j_vals[:2000*12]
            k_vals = k_vals[:2000*12]
...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        value_col_index = df.columns.get_loc('CU_PCT') if 'CU_PCT' in df.columns else (df.columns.get_loc('VALUE') if 'VALUE' in df.columns else 0)
        ...
        if density_method == "Constante":
            ...
            density_column = st.selectbox("Colonne Densité", options=df.columns, index=df.columns.get_loc('DENSITY') if 'DENSITY' in df.columns else 0)
        ...
        col_domain = st.selectbox("Colonne de domaine", options=domain_options, index=domain_options.index('DOMAIN') if 'DOMAIN' in domain_options else 0)
...
if uploaded_file:
    ...
    for _, row in filtered_df.iterrows():
        if pd.notnull(row[col_x]) and pd.notnull(row[col_y]) and pd.notnull(row[col_z]) and pd.notnull(row[col_value]):
            ...
            if density_method == "Variable (colonne)" and density_column and pd.notnull(row[density_column]):
                composite['DENSITY'] = float(row[density_column])
    ...
    with tabs[1]:  # Statistiques
        ...
        with col2:
            if "estimated_blocks" in st.session_state:
                ...
                if density_method == "Variable (colonne)":
                    total_tonnage = sum(block['density'] * block_volume for block in st.session_state.estimated_blocks)
                    avg_density = total_tonnage / total_volume
                else:
                    avg_density = density_value
                    total_tonnage = total_volume * avg_density
        ...
        if "estimated_blocks" in st.session_state:
            ...
            if st.button("Générer le rapport PDF", type="primary"):
                with st.spinner('Génération du rapport en cours...'):
                    ...
                    density_method = st.session_state.density_method
                    density_value = st.session_state.density_value
                    density_column = st.session_state.density_column
